...
namespace MalbersAnimations.Weapons
{
    ...
    public abstract class MWeapon : MDamager, IMWeapon, IAnimatorListener, IMDamager
    {
        ...
        [SerializeField, Flag("Aim Side")] AimSide m_AimSide;
        #endregion
        ...
        public override int ID => weaponType.ID;
        ...
        public virtual bool IsEquiped
        {
            ...
            set
            {
                ...
                if (isEquiped) OnEquiped.Invoke(Owner);
                else
                {
                    Owner = null;                       //Clean the Owner
                    OnUnequiped.Invoke(Owner);
                }
            }
        }
        ...
        public bool IsRightHanded => rightHand;
        public bool IsLefttHanded => !rightHand;

        public Vector3 PositionOffset => rightHand ? positionOffsetR : positionOffsetL;

        public Vector3 RotationOffset => rightHand ? rotationOffsetR : rotationOffsetL;
       ...
       // public UnityEvent OnPlaced = new UnityEvent();
        ...
        internal virtual void SecondaryAttack_Released(IMWeaponOwner RC)
        {
        ...
        {
            if (WeaponRB) WeaponRB.isKinematic = true; //IMPORTANT
    ...
    public abstract class MWeaponEditor : MDamagerEd
    {
        ...
        protected SerializedProperty
            ...
            rotationOffsetIKHand, positionOffsetIKHand, OnAiming, m_Automatic, ChargeCurve, debug, HolsterOffset,
        ...
        protected virtual void DrawExtras()
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                EditorGUILayout.PropertyField(ignoreDamageeM);
            }
            EditorGUILayout.EndVertical();
        ...
        protected virtual void DrawWeapon(bool showAim = true)
        {
            ...
            if (showAim)
            {
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                {
                    EditorGUILayout.PropertyField(m_AimSide, new GUIContent("Side", "Side of the Character the Weapon will aim when Aim is true"));
                    EditorGUILayout.PropertyField(m_AimOrigin, new GUIContent("AimOrigin", "Point where the Aiming will be Calculated.\nAlso for Shootable weapons the point where the Projectiles will come out"));
                }
                EditorGUILayout.EndVertical();
            }


            var hand = rightHand.boolValue ? 1 : 0;
            hand = GUILayout.Toolbar(hand, new string[] { "Left Hand", "Right Hand" });

            rightHand.boolValue = hand == 1;


            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                EditorGUI.indentLevel++;
                offsets = EditorGUILayout.Foldout(offsets, "Offset " + (rightHand.boolValue ? "Right Hand" : "Left Hand"));
                EditorGUI.indentLevel--;
                ...
                if (offsets)
                {
                    EditorGUI.indentLevel++;
                    EditorGUI.indentLevel++;
                    EditorGUILayout.PropertyField(rightHand.boolValue ? positionOffsetR : positionOffsetL, new GUIContent("Position"));
                    EditorGUILayout.PropertyField(rightHand.boolValue ? rotationOffsetR : rotationOffsetL, new GUIContent("Rotation"));
                    EditorGUI.indentLevel--;
                    EditorGUI.indentLevel--;
        ...
        protected virtual void DrawIKWeapon()
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                {
                    EditorGUILayout.LabelField("(The " + (rightHand.boolValue ? "Left Hand" : "Right Hand") + " is the auxiliar Hand)");

...
namespace MalbersAnimations.HAP
{
    ...
    {
        private const string FreeBothtHands = "FreeBothtHands";
        ...
        private AimSide defaultAimSide;
        ...
        public int WeaponAction
        {
            ...
            {
                weaponAction = value;
                if (debug) Debug.Log($"<B>{name}:<color=green> [Weapon Action] -> [{WA.WValue(value)}]</color></b> T{Time.time:F3}");
                ...
                if (weaponAction == WA.None) SendMessage(FreeBothtHands);
            }
        }
        ...
        private void SetWeapon(bool new_Weapon)
        {
            if (new_Weapon)
            {
                ...
                Aimer.AimSide = m_weapon.AimSide;
            }
            else
            {
                m_weapon.WeaponAction -= Action;
                m_weapon.OnCharged.RemoveListener(SetWeaponCharge);
                Aimer.AimSide = defaultAimSide;
                Aimer.IgnoreTransform = null;
                Aimer.AimOrigin = null;
      ...
      //  public bool RiderCanAttack => MTools.CompareOR(WeaponAction, WA.Idle, WA.Preparing, WA.AimLeft, WA.AimRight);
        ...
        public bool Aim
        {
            private set
            {
                if (!WeaponIsActive || Weapon.AimSide == 0) return; //Do nothing if the weapon is not active
                ...
                if (aim != value)
                {
                    ...
                    //if (StrafeOnTarget)
                    //    Rider.Montura.Animal.UseCameraInput = aim || DefaultMonturaCamInput;
                    ...
                    else
                    {
                       ...
                       // Weapon.ResetCharge(); //Reset the Charge of the wapon

...
namespace MalbersAnimations.Weapons
{
    public class MShootable : MWeapon, IShootableWeapon, IThrower
    {
        ...
        {/*NO WEAPON*/ }
        internal override void SecondaryAttack_Released(IMWeaponOwner RC)
        {/*NO WEAPON*/ }
        ...
        internal override void Attack_Charge(IMWeaponOwner RC, float time)
        {
            if (MainInput)
            {
                ...
                {
                    MainAttack_Start(RC);
                    if (debug) Debug.Log($"{name}:<color=white> <b>[Charge Started] </b> [{ProjectileInstance.name}]  </color>");  //Debug
                }
    ...
    public class MShootableEditor : MWeaponEditor
    {
        SerializedProperty  m_AmmoInChamber, m_Ammo, m_ChamberSize, releaseProjectile, m_Projectile, AimLimit, 
            m_AutoReload, InstantiateProjectileOfFire, ProjectileParent,
